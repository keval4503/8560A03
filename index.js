...
const NachosOrder = require("./assignment2Nachos");
...
app.post("/payment/:phone", (req, res) => {
  ...
  for (let n = 0; n < aReply.length; n++) {
    if (oSocket) {
      ...
      console.log("twilio code would go here");
    }
  }
...
app.post("/payment", (req, res) => {
  ...
  oOrders[sFrom] = new NachosOrder(sFrom);
...
app.post("/sms", (req, res) => {
  ...
  if (!oOrders.hasOwnProperty(sFrom)) {
    oOrders[sFrom] = new NachosOrder(sFrom, sUrl);
  }
  if (oOrders[sFrom].isDone()) {
    delete oOrders[sFrom];
  }
